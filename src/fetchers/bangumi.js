const CALENDAR_API = 'https://api.bgm.tv/calendar'
const EPISODES_API = 'https://api.bgm.tv/v0/episodes'
const USER_AGENT = 'bangumi-calendar-plugin/1.0 (https://github.com/bangumi/api)'
const WEEKDAY_CN = ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六']
const EPISODE_LIMIT = 100
const EPISODE_CONCURRENCY = 4
const MAX_EPISODE_PAGES = 5

export async function getTodayBangumi(date = new Date()) {
	const calendar = await fetchCalendar()
	const day = pickCalendarDay(calendar, date)

	if (!day) {
		throw new Error('没有找到今天的番剧日历数据')
	}

	const data = normalizeCalendarDay(day, date)
	data.items = await enrichItemsWithEpisodeInfo(data.items, date)
	return data
}

export async function fetchCalendar(fetchImpl = globalThis.fetch) {
	const data = await fetchJson(CALENDAR_API, { fetchImpl })
...
export async function fetchJson(url, { fetchImpl = globalThis.fetch, timeout = 15000 } = {}) {
	if (typeof fetchImpl !== 'function') {
		throw new Error('当前 Node.js 不支持 fetch，请升级到 Node.js 18 或更高版本')
	}
...
export function pickCalendarDay(calendar, date = new Date()) {
	const weekdayId = getBangumiWeekdayId(date)
	return calendar.find((day) => Number(day?.weekday?.id) === weekdayId)
}

export function getBangumiWeekdayId(date = new Date()) {
	const day = date.getDay()
	return day === 0 ? 7 : day
}

export function normalizeCalendarDay(day, date = new Date()) {
...
async function enrichItemsWithEpisodeInfo(items, date) {
	return await mapLimit(items, EPISODE_CONCURRENCY, async (item) => {
		try {
			const episodeText = await getTodayEpisodeText(item.id, date)
			return {
				...item,
				episodeText
			}
...
async function getTodayEpisodeText(subjectId, date) {
	if (!subjectId) return ''

	const firstPage = await fetchEpisodesPage(subjectId, 0)
	const total = Number(firstPage?.total || 0)
	const firstMatch = pickTodayEpisode(firstPage?.data, date)
	...
	while (offset > 0 && pageCount < MAX_EPISODE_PAGES) {
		const page = await fetchEpisodesPage(subjectId, offset)
		const match = pickTodayEpisode(page?.data, date)
...
async function fetchEpisodesPage(subjectId, offset) {
	const url = `${EPISODES_API}?subject_id=${encodeURIComponent(subjectId)}&type=0&limit=${EPISODE_LIMIT}&offset=${offset}`
	return await fetchJson(url, { timeout: 12000 })
}

function pickTodayEpisode(episodes, date) {
